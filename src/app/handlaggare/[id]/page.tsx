...
import { ATGARD_KRAVS, kanTillampaForslag, redigerbaraFalt } from '../../../lib/handlaggare-vy.ts';
import { delaMetadata, type Faltrad } from '../../../lib/registrator-vy.ts';
import { hamtaBeroenden } from '../../../server/beroenden.ts';
import { Faltredigering } from './Faltredigering.tsx';
import { NyGranskning } from './NyGranskning.tsx';
import { TillampaForslag } from './TillampaForslag.tsx';
...
export default async function Handlaggardetalj({
  params,
  searchParams,
}: {
  ...
  const { lager, logg, katalog } = hamtaBeroenden();
  const post = lager.hamta(decodeURIComponent(id));
  if (post === undefined) notFound();

  const { dokument } = post;
  const metadata = delaMetadata(dokument);
  const redigerbara = redigerbaraFalt(dokument);
  const historik = flik === 'logg' ? await logg.lasForDokument(dokument.id) : [];
  const kanAtgardas = post.granskningsstatus === ATGARD_KRAVS;
  const flikhref = (f: Flik) => `/handlaggare/${encodeURIComponent(dokument.id)}?flik=${f}`;

  return (
    <>
      <p>
        <Link href="/handlaggare">← Åtgärdskö</Link>
      </p>
      ...
      {kanAtgardas && <NyGranskning dokumentId={dokument.id} />}
        ...
        {post.fynd.map((fynd) => (
          <article key={`${fynd.regelId}-${fynd.metod}`} className="fynd">
            ...
            {kanAtgardas &&
              ...
              kanTillampaForslag(fynd.regelId, katalog) && (
              <TillampaForslag dokumentId={dokument.id} regelId={fynd.regelId} />
            )}
          </article>
        ))}
      </section>
      ...
      {flik === 'detaljer' &&
        (kanAtgardas ? (
          <Faltredigering
            key={`detaljer-${dokument.id}`}
            dokumentId={dokument.id}
            falt={redigerbara.detaljer}
          />
        ) : (
          <Falttabell rader={metadata.detaljer} />
        ))}
      {flik === 'kontakter' &&
        (kanAtgardas ? (
          <Faltredigering key={`kontakter-${dokument.id}`} dokumentId={dokument.id} falt={redigerbara.kontakter} />
        ) : (
          <Falttabell rader={metadata.kontakter} />
        ))}
